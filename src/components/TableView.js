            ...
            <table style={_.assign({}, {border: '1px solid #ccc', borderCollapse: 'collapse'}, widthStyle ? widthStyle : {})}>
                <thead>
                {
                    _.map(tableHeader, (headerList, trindex) => {
                        return (
                            <tr key={'thead-tr-' + trindex} style={{backgroundColor: colorsMap.C02,textAlign: 'center' }}>
                                {
                                    _.map(headerList, (header, thindex) => {
                                        ...
                                        return (
                                            <th key={'thead-th-' + thindex}
                                                ...
                                                style={_.assign({}, header.height ? {height: header.height} : {}, {textAlign: 'center', padding:'6px 30px', borderColor: colorsMap.C04, position: 'relative', whiteSpace: 'nowrap', fontWeight: 'bold'})}
                                    ...
                                    _.map(headSeq, (id, tdindex) => {
                                        ...
                                        return (
                                            <td key={'tbody-td-' + tdindex}
                                                ...
                                                style={_.assign({}, tdStyle, {padding:'6px 30px', textAlign:'center', borderColor: colorsMap.C04, whiteSpace: 'nowrap'}, _.isObject(rowData[id]) ? {cursor: 'pointer'} : {})}
                                                onClick={(_.isObject(rowData[id]) && !rowData[id].props) ? this.handleOnClick.bind(this, id, rowData) : this.emptyFn()}
                                                >
                                                {
                                                    (_.isObject(rowData[id]) && rowData[id]['content']) ? (
                                                        <OverlayTrigger
                                                            placement={'top'}
                                                            trigger={['hover', 'focus']}
                                                            overlay={popoverFactory({content: rowData[id]['content']})}
                                                            >
                                                            <span style={{cursor: 'pointer', color: colorsMap.B03}}>{tdData}</span>
                                                        </OverlayTrigger>
                                                    ) : tdData
                                                }
                                            </td>
...
// --------------------mork 数据----------------------------
let tableHeader = [[], []]; let tableData = []
tableHeader[0] = [{id: 'class', name: '班级', rowSpan: 2, freeze:'left', height: '80px'}]
_.each(_.range(20), num => {
    tableHeader[0].push({name: num + '档', colSpan: 2})
    tableHeader[1].push({name: '人数', id: 'count'+num})
    tableHeader[1].push({name: '累计人数', id: 'numcount'+num})
})
_.each(_.range(10), rowNum => {
    let rowData = {}
    rowData['class'] = {value: 10 + rowNum, drillFilter: '钻取人数'}
    _.each(_.range(20) , num => {
        rowData['count' + num ] = 10 + rowNum;
        rowData['numcount' + num ] = 20 + rowNum + num;
    })
